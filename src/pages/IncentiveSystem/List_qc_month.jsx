...
import {
    ApproveAPi, checkSavedApi, ConfirmPayDate, QcLogApi,
    StoreQcMonthApi, UpdateQcMonthApi
} from "../../api/QcMonth.js";
import {
...
import {
    faCalculator, faCheck, faFileExport, faFloppyDisk,
    faPaperPlane, faPenToSquare, faPrint, faXmark
...
function List_qc_month() {
    ...
    const [isEditHR, setIsEditHR] = useState(false);
    const [prevEditHR, setPrevEditHR] = useState();
        ...
        QcLogApi(year, month, status)
            .then(({ data, status }) => {
                if (status === 200) {
                    ...
                    setPrevEditHR(data.amount_qc_users)
    ...
    //ฟังก์ชั่นการแก้ไขของ HR
    const handleEditHR = () => {
        console.log('Datas:', prevEditHR);
        AlertInfoWithQuestion({
            title: 'แก้ไขข้อมูล', text: 'คุณต้องการแก้ไขข้อมูลหรือไม่?', textConfirm: 'ใช่', textCancel: 'ไม่',
            onPassed: async (confirm) => {
                if (confirm) {
                    const updated_new = datas.map(({ paystatus, empqc, payremark }) => ({
                        paystatus: paystatus === 'yes' ? 'yes' : 'no',
                        empqc,
                        payremark: payremark ? payremark : null
                    }));
                    try {
                        setLoading(true);
                        const { status } = await axiosClient.post('/incentive/qc_month/update_paystatus', {
                            inc_id, list: updated_new
                        })
                        if (status === 200) {
                            ...
                            (<>
                                {
                                    data_team.status !== 'complete' && (
                                        <button onClick={handleCalculate}
                                            disabled={loading || currentUser.emp_role === 'HR'}
                                ...
                                <>
                                    ...
                                    <button onClick={RedirectToEdit}
                                        ...
                                        className={`text-end btn btn-primary mr-3 ${currentUser.emp_role === 'QC' ? '' : 'd-none'}`}>
                                        <FontAwesomeIcon icon={faPenToSquare} className={'mr-1'} />
                                        <span>แก้ไข (สำหรับ QC)</span>
                                    </button>

                                    <button
                                        onClick={() => setIsEditHR(true)}
                                        disabled={
                                            loading ||
                                            (currentUser.emp_role === 'QC' || data_team.status !== 'wait')
                                        }
                                        className={`text-end btn btn-primary mr-3 ${currentUser.emp_role === 'HR' ? '' : 'd-none'}`}>
                                        <FontAwesomeIcon icon={faPenToSquare} className={'mr-1'} />
                                        <span>แก้ไข (สำหรับ HR)</span>
                                    </button>

                                    {
                                        (data_team.status === 'approve' && currentUser.emp_role === 'HR') && (
                                            <button onClick={onConfirmPayDate} disabled={loading}
                                                ...
                                                <span>ยืนยันการจ่าย (สำหรับ HR)</span>
                                            </button>
                                        )
                                    }
                                    ...
                                    {currentUser.emp_role === 'QC' ? (
                                        data_team.status === 'active' && (
                                            <button onClick={onApproveQC} disabled={loading}
                                                className={'text-end btn mr-3 text-white bg-info'}>
                                                <FontAwesomeIcon icon={faPaperPlane} className={'mr-1'} />
                                                <span>ส่งอนุมัติ (สำหรับ QC)</span>
                                            </button>
                                        )
                ...
                <div className={'card-body'}>
                    ...
                    {isEditHR && (
                        <div className="d-flex justify-content-center mt-3">
                            ...
                            >
                                {loading ? 'กำลังบันทึก...' : <><FontAwesomeIcon icon={faFloppyDisk} className={'mr-2'} />
                                    บันทึกการแก้ไข
                                </>}
                            </button>
                        </div>
